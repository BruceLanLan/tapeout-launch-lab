...
function burnLift(t, burn, thin) {
  if (burn < 0.01) return 1;
  const rise = 1 - Math.exp(-Math.max(t, 0) / 72);
  return 1 + thin * burn * 3.2 * rise;
}
...
function simulateTape(times, params, depth, demandBoost) {
  ...
  for (const h of times) {
    ...
    if (day > lastDay) {
      ...
      const buy = quote0 * 0.012 * params.demand * demandBoost * attention * daySpan;
      if (buy > 0) {
        const k = amm.token * amm.quote;
        amm.quote += buy;
...
function runSim(params) {
  ...
  return times.map((tHours, i) => ({
    ...
    prices: {
      pump: casinoPrice(tHours, params.demand, CASINO.pump) * burnLift(tHours, params.fuelBurn, 1),
      four: casinoPrice(tHours, params.demand, CASINO.four) * burnLift(tHours, params.fuelBurn, 1),
      flap: casinoPrice(tHours, params.demand, CASINO.flap) * burnLift(tHours, params.fuelBurn, 0.45),
      pons: casinoPrice(tHours, params.demand, CASINO.pons) * burnLift(tHours, params.fuelBurn, 1),
      long: casinoPrice(tHours, params.demand, CASINO.long) * burnLift(tHours, params.fuelBurn, 0.4),
      virtuals: casinoPrice(tHours, params.demand, CASINO.virtuals) * burnLift(tHours, params.fuelBurn, 0.25),
      tapehub: th.price[i],
...
function renderControls() {
  ...
  document.getElementById("controls").innerHTML = `
    ...
    <label class="block">回购销毁 <span class="val">${p.fuelBurn.toFixed(2)}</span></label>
